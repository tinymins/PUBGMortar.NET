...
应用程序**自动检测**你的屏幕分辨率并计算相应参数：
- **CenterPixelY** - 根据屏幕高度自动计算
- **MaxDegree** - 根据屏幕宽高比使用 PUBG 的 Hor+ FOV 系统自动计算

这意味着应用程序可以在任何分辨率（1080p、1440p、4K、超宽屏等）下正常工作，无需手动调整。

> 💡 计算基于 PUBG 默认水平 FOV（约 80°）。如果你在游戏中修改了 FOV，仰角检测可能会有轻微偏差。
